      ...
      <TouchableOpacity
        ...
        <Text
          style={[
            ...
            value === opt.value && styles.toggleBtnTextActive,
          ]}>
          {opt.label}
        </Text>
        {opt.sub && (
          <Text
            style={[
              styles.toggleBtnSub,
              value === opt.value && styles.toggleBtnSubActive,
            ]}>
            {opt.sub}
          </Text>
        )}
...
const Stepper = React.memo(
  ({value, onDecrement, onIncrement, min = 0, themedColors, styles}: any) => (
    <View style={styles.stepperWrap}>
      <TouchableOpacity
        ...
        <Icon
          name="remove"
          size={18}
          color={value <= min ? themedColors.textHint : themedColors.text}
        />
      </TouchableOpacity>
...
const TimeStepper = React.memo(
  ({value, onChange, themedColors, styles}: any) => {
...
const AmountField = React.memo(
  ({label, value, onChange, themedColors, styles}: any) => (
    ...
    <View>
      ...
      <View style={styles.tenantToggleRow}>
        <TouchableOpacity
          style={[
            styles.tenantTabBtn,
            form.tenantTab === 'existing' && styles.tenantTabBtnActive,
          ]}
          onPress={() => updateForm({tenantTab: 'existing'})}>
          <Text
            style={[
              styles.tenantTabText,
              form.tenantTab === 'existing' && styles.tenantTabTextActive,
            ]}>
        ...
        <TouchableOpacity
          style={[
            styles.tenantTabBtn,
            form.tenantTab === 'new' && styles.tenantTabBtnActive,
          ]}
          onPress={() => updateForm({tenantTab: 'new'})}>
          <Text
            style={[
              styles.tenantTabText,
              form.tenantTab === 'new' && styles.tenantTabTextActive,
            ]}>
      ...
      {form.tenantTab === 'existing' ? (
        <View style={{zIndex: 10}}>
          ...
          <TouchableOpacity
            style={[
              ...
              isCustomerDropdownOpen && styles.dropdownTriggerActive,
            ]}
                    ...
                    <TouchableOpacity
                      ...
                      <View style={{flex: 1}}>
                        <Text
                          style={[
                            styles.customerName,
                            selectedCustomer?.id === c.id && {color: '#fff'},
                          ]}>
                          {c.full_name}
                        </Text>
                        <Text
            ...
            <View style={styles.selectedCustomerSummary}>
              ...
              <View style={{flex: 1}}>
                ...
                <Text style={styles.customerPhone}>
                  {selectedCustomer.phone || selectedCustomer.id_number}
                </Text>
              </View>
        ...
        <View style={{gap: 10}}>
          <TouchableOpacity style={styles.scanBtn}>
            <Text style={styles.scanBtnText}>{t('booking.form.scanning')}</Text>
          </TouchableOpacity>
          <FieldInput
            fieldKey="fullName"
            value={form.fullName}
            onChangeText={(v: any) => updateForm({fullName: v})}
          ...
          <FieldInput
            fieldKey="idCard"
            value={form.idCard}
            onChangeText={(v: any) => updateForm({idCard: v})}
            placeholder={t('booking.form.idCard')}
            themedColors={themedColors}
            styles={styles}
          />
        </View>
...
const ServiceSelectionModal = React.memo(
  ({
    visible,
    onClose,
    availableServices,
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>
              {t('booking.form.serviceModalTitle')}
            </Text>
          ...
          <ScrollView style={{maxHeight: 400}}>
            {availableServices.length === 0 ? (
              <Text
                ...
                {t('booking.form.noAvailableServices')}
              </Text>
            ) : (
              availableServices.map((svc: any) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (ext) {
                      ...
                      updateForm({
                        services: [
                          ...form.services,
                          {...svc, qty: 1, selected: true},
                        ],
                      });
                    }
                  }}>
                  <View style={styles.svcSelectIcon}>
                    <Icon
                      name="room-service"
                      size={20}
                  ...
                  <View style={{flex: 1}}>
                    ...
                    <Text style={styles.svcSelectPrice}>
                      {formatVND(svc.unitPrice)} / {svc.unit}
                    </Text>
                  </View>
      ...
      <View>
        ...
        {form.services.length === 0 ? (
          <View
            ...
            <Text style={{color: themedColors.textHint}}>
              {t('booking.form.noServices')}
            </Text>
          </View>
        ) : (
          <View style={styles.card}>
            {form.services.map((svc: Service, idx: number) => (
              <View
                ...
                style={[
                  styles.svcItem,
                  idx < form.services.length - 1 && styles.svcItemBorder,
                ]}>
                <View style={{flex: 1}}>
...
const LongTermForm = React.memo(
  ({form, updateForm, t, themedColors, styles, isDark}: any) => {
      ...
      <>
        ...
        <TouchableOpacity
          ...
          <Text style={{color: themedColors.text}}>
            {form.contractStart || t('booking.form.selectStartDate')}
          </Text>
          <Icon
            name="calendar-today"
            size={18}
        ...
        <TouchableOpacity
          style={[
            styles.dropdownTrigger,
            isDurationOpen && styles.dropdownTriggerActive,
          ]}
          onPress={() => setIsDurationOpen(!isDurationOpen)}>
          <Text style={{flex: 1, color: themedColors.text}}>
            {durationOptions.find(o => o.value === form.contractDuration)
              ?.label || 'Chọn thời hạn'}
          </Text>
          <Icon
            name="arrow-drop-down"
            ...
            {durationOptions.map((opt, idx) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  borderBottomWidth:
                    idx === durationOptions.length - 1 ? 0 : 0.5,
                  borderBottomColor: themedColors.border,
                  backgroundColor:
                    form.contractDuration === opt.value
                      ? isDark
                        ? '#2D3748'
                        : '#F0F7FF'
                      : 'transparent',
                ...
                onPress={() => {
                  updateForm({contractDuration: opt.value});
                  ...
                  style={{
                    ...
                    color:
                      form.contractDuration === opt.value
                        ...
                        : themedColors.text,
                    fontWeight:
                      form.contractDuration === opt.value ? '600' : '400',
                  }}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
        ...
        <View style={{height: 12}} />
        <AmountField
          label={t('booking.form.deposit')}
          value={form.deposit}
          onChange={(v: any) => updateForm({deposit: v})}
          themedColors={themedColors}
          styles={styles}
        />
        <AmountField
          label={t('booking.form.monthlyRent')}
          value={form.monthlyPrice}
          onChange={(v: any) => updateForm({monthlyPrice: v})}
          themedColors={themedColors}
          styles={styles}
        />
        <View style={{height: 12}} />
        ...
        <View style={styles.dateRow}>
          <View style={{flex: 1}}>
            ...
            <FieldInput
              fieldKey="electricStart"
              value={form.electricStart}
              onChangeText={(v: any) => updateForm({electricStart: v})}
              placeholder="0"
              keyboardType="numeric"
              themedColors={themedColors}
              styles={styles}
          ...
          <View style={{flex: 1}}>
            ...
            <FieldInput
              ...
              onChangeText={(v: any) => updateForm({waterStart: v})}
              placeholder="0"
              keyboardType="numeric"
              themedColors={themedColors}
              styles={styles}
        ...
        <CalendarModal
          ...
          onConfirm={d => {
            setIsCalOpen(false);
            updateForm({contractStart: d.toISOString().split('T')[0]});
          }}
...
const ShortTermForm = React.memo(
  ({form, updateForm, t, themedColors, styles}: any) => {
    ...
    return (
      <>
        ...
        <View style={styles.dateRow}>
          ...
          <View style={{flex: 1}}>
            ...
            <TimeStepper
              ...
              onChange={(v: any) => updateForm({checkinTime: v})}
              themedColors={themedColors}
              styles={styles}
            />
          </View>
        ...
        <View style={styles.dateRow}>
          ...
          <View style={{flex: 1}}>
            ...
            <TimeStepper
              ...
              onChange={(v: any) => updateForm({checkoutTime: v})}
              themedColors={themedColors}
              styles={styles}
            />
          </View>
        ...
        <View style={styles.utilRow}>
          <View style={{flex: 1}}>
            ...
            <Stepper
              value={form.adults}
              onDecrement={() =>
                updateForm({adults: Math.max(1, form.adults - 1)})
              }
          ...
          <View style={{flex: 1}}>
            ...
            <Stepper
              value={form.children}
              onDecrement={() =>
                updateForm({children: Math.max(0, form.children - 1)})
              }
        ...
        <CalendarModal
          ...
          onConfirm={d => {
            setIsInOpen(false);
            updateForm({checkinDate: d.toISOString().split('T')[0]});
          }}
        ...
        <CalendarModal
          ...
          onConfirm={d => {
            setIsOutOpen(false);
            updateForm({checkoutDate: d.toISOString().split('T')[0]});
          }}
...
const LongTermSummary = React.memo(
  ({
    form,
    room,
    selectedCustomer,
    ...
    const serviceTotal = form.services.reduce(
      (sum: number, s: any) => sum + s.qty * s.unitPrice,
      0,
    );
        ...
        <View style={styles.summaryContent}>
          <View style={styles.summarySectionHeader}>
            <Text style={styles.summarySectionTitle}>
              {t('booking.summary.roomSummary')}
            </Text>
          </View>
          <View style={styles.summaryRoomCard}>
            <Image
              source={{uri: PLACEHOLDER_IMAGE}}
              style={styles.summaryRoomImage}
            />
            <View style={styles.summaryRoomInfo}>
              <Text style={styles.summaryRoomTag}>
                {t('booking.summary.roomSummary')}
              </Text>
          ...
          <View style={styles.summarySectionHeader}>
            <Text style={styles.summarySectionTitle}>
              {t('booking.summary.customerInfo')}
            </Text>
            <TouchableOpacity onPress={onEdit}>
              <Text style={styles.summaryEditBtn}>
          ...
          <View style={styles.summaryCustomerCard}>
            ...
            <View style={styles.summaryCustomerInfo}>
              <Text style={styles.summaryCustomerName}>
                {selectedCustomer?.full_name || form.fullName}
              </Text>
              <Text style={styles.summaryCustomerPhone}>
                {selectedCustomer?.phone || form.phone}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.summaryDetailGrid}>
            <View style={styles.summaryGridRow}>
              ...
              <View style={styles.summaryGridItem}>
                ...
                <Text style={styles.summaryGridValue}>
                  {form.contractDuration === '1'
                    ? '12 tháng'
                    : form.contractDuration === '2'
                    ? '24 tháng'
            ...
            <View style={styles.summaryGridRow}>
              <View style={styles.summaryGridItem}>
                <Text style={styles.summaryGridLabel}>
                  {t('booking.form.electric')} ({t('booking.form.initialMeter')}
                  )
                </Text>
                <Text style={styles.summaryGridValue}>
                  {form.electricStart} kWh
                </Text>
              </View>
              <View style={styles.summaryGridItem}>
                <Text style={styles.summaryGridLabel}>
                  {t('booking.form.water')} ({t('booking.form.initialMeter')})
                </Text>
                <Text style={styles.summaryGridValue}>
                  {form.waterStart} m³
                </Text>
          ...
          <View style={styles.summaryHighlightCard}>
            <Text style={styles.summaryHighlightLabel}>
              {t('booking.form.monthlyRent')}
            </Text>
            ...
            <>
              ...
              {form.services.map((svc: any) => (
                <View key={svc.id} style={styles.summarySvcItem}>
                  ...
                  <Text style={styles.summarySvcPrice}>
                    {formatVND(svc.unitPrice)}/tháng
                  </Text>
                </View>
              ))}
              <View style={{height: 12}} />
          ...
          <View style={styles.summaryTotalCard}>
            <Text style={styles.summaryTotalTitle}>
              {t('booking.summary.totalInitial')}
            </Text>
            <View style={styles.summaryTotalRow}>
              <Text style={styles.confSummaryTotalLabel}>
                {t('booking.summary.roomRentMonth')}
              </Text>
              <Text style={styles.confSummaryTotalValue}>
                {formatVND(form.monthlyPrice)}
              </Text>
            </View>
            <View style={styles.summaryTotalRow}>
              <Text style={styles.confSummaryTotalLabel}>
                {t('booking.summary.depositMonth')}
              </Text>
              <Text style={styles.confSummaryTotalValue}>
            ...
            <View style={styles.summaryTotalRow}>
              <Text style={styles.confSummaryTotalLabel}>
                {t('booking.summary.serviceMonthly')}
              </Text>
              <Text style={styles.confSummaryTotalValue}>
            ...
            <View style={styles.summaryGrandTotalRow}>
              <Text style={styles.confSummaryGrandTotalLabel}>
                {t('booking.summary.total')}
              </Text>
              <Text style={styles.confSummaryGrandTotalValue}>
          ...
          <TouchableOpacity
            ...
            <View
              style={[
                ...
                agreedTerms && styles.summaryCheckboxChecked,
              ]}>
              {agreedTerms && <Icon name="check" size={14} color="#fff" />}
            </View>
            <Text style={styles.summaryCheckboxText}>
              {t('booking.summary.termsLong')}
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[
              styles.summarySubmitBtn,
              (!agreedTerms || confirming) && styles.summarySubmitBtnDisabled,
            ]}
            onPress={onConfirm}
            ...
            {confirming ? (
              ...
              <Text style={styles.summarySubmitBtnText}>
                {t('booking.summary.btnSign')}
              </Text>
            )}
          </TouchableOpacity>
...
const ShortTermSummary = React.memo(
  ({
    form,
    room,
    selectedCustomer,
    ...
    return (
      <ScrollView style={styles.scrollView}>
        <View style={styles.summaryContent}>
          ...
          <View style={styles.summarySectionHeader}>
            <Text style={styles.summarySectionTitle}>
              {t('booking.sections.stayDetails')}
            </Text>
          </View>
          <View style={styles.summaryDetailGrid}>
            <View style={styles.summaryGridRow}>
            ...
            <View style={styles.summaryGridRow}>
              <View style={styles.summaryGridItem}>
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  ...
                  <Text style={{fontSize: 14, color: '#1A1A1A', marginLeft: 6}}>
                    {form.adults} {t('booking.form.adults')}, {form.children}{' '}
            ...
            <>
              ...
              {form.services.map((svc: any) => (
                <View key={svc.id} style={styles.summarySvcItem}>
                  ...
                  <Text style={styles.summarySvcName}>{svc.name}</Text>
                  <Text style={styles.summarySvcPrice}>
                    {formatVND(svc.qty * svc.unitPrice)}
                  </Text>
                </View>
              ))}
              <View style={{height: 16}} />
          ...
          <View style={styles.summaryTotalCard}>
            <Text style={styles.summaryTotalTitle}>
              {t('booking.summary.paymentDetail')}
            </Text>
            <View style={styles.summaryTotalRow}>
              <Text style={styles.confSummaryTotalLabel}>
                {t('booking.summary.roomRentNights', {nights})}
              </Text>
              <Text style={styles.confSummaryTotalValue}>
                {formatVND(roomTotal)}
              </Text>
          ...
          <TouchableOpacity
            style={[
              styles.summarySubmitBtn,
              confirming && styles.summarySubmitBtnDisabled,
            ]}
            onPress={onConfirm}
            disabled={confirming}>
            {confirming ? (
...
const BookingScreen = ({route, navigation, ...props}: any) => {
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        confSummaryTotalValue: {
          fontSize: 14,
          color: '#1A1A1A',
          fontWeight: '600',
        },
        confSummaryGrandTotalLabel: {
          fontSize: 16,
          fontWeight: 'bold',
          color: '#1A1A1A',
        },
        confSummaryGrandTotalValue: {
          fontSize: 20,
          fontWeight: 'bold',
          color: themedColors.primary,
        },
  ...
  const processFinalSubmission = async () => {
    ...
    if (form.tenantTab === 'new' && (!form.fullName || !form.phone)) {
      Alert.alert('Lỗi', 'Vui lòng nhập đầy đủ họ tên và SĐT khách mới');
      return;
    }
    ...
    try {
      if (form.stayType === 'long_term') {
        const duration =
          form.contractDuration === '1 năm'
            ? 12
            : form.contractDuration === '2 năm'
            ? 24
        ...
        <ScrollView
          ...
          <ToggleGroup
            ...
            options={[
              {
                value: 'long_term',
                label: t('booking.sections.longTerm'),
                sub: t('booking.sections.longSub'),
              },
              {
                value: 'short_term',
                label: t('booking.sections.shortTerm'),
                sub: t('booking.sections.shortSub'),
              },
            ]}
            styles={styles}
          />
          ...
          <View style={styles.summaryCard}>
            ...
            <View style={styles.summaryTotal}>
              ...
              <Text style={styles.summaryTotalValue}>
                {formatVND(
                  (form.stayType === 'long_term'
                    ? form.monthlyPrice + form.deposit
                    : room.price) +
                    form.services.reduce(
                      (sum, s) => sum + s.qty * s.unitPrice,
                      0,
