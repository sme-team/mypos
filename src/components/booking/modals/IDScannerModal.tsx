...
export const IDScannerModal: React.FC<IDScannerModalProps> = ({
        ...
        if (firstCode.value) {
          ...
          if (result) {
            ...
            Alert.alert(
              t('idScanner.invalid'),
              `${t(
                'idScanner.invalidDesc',
              )}\n\nRaw: ${firstCode.value.substring(0, 50)}...`,
              [
                {
                  text: t('idScanner.btnRetry'),
  ...
  const handlePickImage = async () => {
    ...
    try {
      const visionResult = await TextRecognition.recognize(
        result.assets[0].uri,
      );
      ...
      if (!fullName) {
        const namePattern = lines.find(
          l =>
            l.length > 5 &&
            l.length < 50 &&
            !/\d/.test(l) &&
            !/\b(SỐ|SO|NO\.?|N[Oo]|ID|CCCD|IDENTITY|NUMBER|NUM|CARD|CMT)\b/i.test(
              l,
            ) && // Loại trừ nhiều từ khóa hơn
            /^[A-ZÀÁẢÃẠÂẦẤẨẪẬĂẰẮẲẴẶĐÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴ\s]+$/.test(
              l,
            ),
        );
        if (namePattern) {
      ...
      if (!fullName) {
        const wordPattern = lines.find(
          l =>
            l.length > 5 &&
            l.length < 50 &&
            !/\d/.test(l) &&
            !/\b(SỐ|SO|NO\.?|N[Oo]|ID|CCCD|IDENTITY|NUMBER|NUM|CARD|CMT|FULL|NAME|GIOI|TINH|NGAY|SINH|DIA|CHI|QUE|QUAN)\b/i.test(
              l,
            ) &&
            /^[A-Z][a-zÀÁẢÃẠÂẦẤẨẪẬĂẰẮẲẴẶĐÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴ]+(\s[A-Z][a-zÀÁẢÃẠÂẦẤẨẪẬĂẰẮẲẴẶĐÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴ]+){1,3}$/.test(
              l,
            ),
        );
        if (wordPattern) {
      ...
      if (genderMatch) {
        ...
        if (genderText.includes('nam') || genderText.includes('male'))
          gender = 'Nam';
        else if (genderText.includes('nữ') || genderText.includes('female'))
          gender = 'Nữ';
      }
      ...
      if (!gender) {
        for (let i = 0; i < lines.length; i++) {
          ...
          if (
            ...
            else if (checkText.includes('nữ') || checkText.includes('nu'))
              gender = 'Nữ';
            break;
          }
        }
      ...
      if (!address) {
        const addressLikeLine = lines.find(
          l =>
            /\d/.test(l) && // Có số (số nhà)
            (l.toLowerCase().includes('đường') ||
              l.toLowerCase().includes('phố') ||
              l.toLowerCase().includes('phường') ||
              l.toLowerCase().includes('quận') ||
              l.toLowerCase().includes('huyện') ||
              l.toLowerCase().includes('thành phố') ||
              l.toLowerCase().includes('tỉnh') ||
              l.toLowerCase().includes('street') ||
              l.toLowerCase().includes('district') ||
              l.toLowerCase().includes('ward')),
        );
  ...
  const updateEditableField = useCallback(
    (field: keyof CCCDData, value: string) => {
  ...
  if (scannedData) {
    ...
    const hasMissingData =
      !data.dateOfBirth || !data.gender || !data.address || !data.placeOfOrigin;

    return (
      <Modal visible={visible} animationType="slide" transparent={false}>
        <SafeAreaView
          style={[styles.container, {backgroundColor: themedColors.bg}]}>
          {/* Header */}
          <View style={styles.header}>
            <TouchableOpacity
              onPress={editMode ? handleCancelEdit : handleRescan}
            ...
            <Text style={[styles.headerTitle, {color: themedColors.text}]}>
              {editMode
                ? t('idScanner.editTitle')
                : t('idScanner.confirmTitle')}
            </Text>
            <View style={{width: 40}} />
          </View>
          ...
          <ScrollView
              ...
              style={[
                styles.successBadge,
                {
                  backgroundColor: editMode
                    ? themedColors.warningLight
                    : themedColors.primaryLight,
                },
              ]}>
              <Icon
                name={editMode ? 'edit' : 'verified-user'}
                size={48}
                color={editMode ? themedColors.warning : themedColors.primary}
              />
              <Text
                style={[
                  styles.successTitle,
            ...
            {hasMissingData && !editMode && (
              <View
                style={[
                  styles.warningBadge,
                  {backgroundColor: themedColors.warningLight},
                ]}>
                <Icon name="warning" size={20} color={themedColors.warning} />
                <Text
                  style={[styles.warningText, {color: themedColors.warning}]}>
            ...
            <View
              ...
              {editMode ? (
                ...
                <>
                  <EditRow
                    icon="badge"
                    label={t('idScanner.idCard')}
                    value={data.idCard}
                    onChange={(val: string) =>
                      updateEditableField('idCard', val)
                    }
                    themedColors={themedColors}
                  />
                  <EditRow
                    icon="person"
                    label={t('idScanner.fullName')}
                    value={data.fullName}
                    onChange={(val: string) =>
                      updateEditableField('fullName', val)
                    }
                    themedColors={themedColors}
                  />
                  ...
                  <EditRow
                    icon="cake"
                    label={t('idScanner.dob')}
                    value={data.dateOfBirth}
                    placeholder="DD/MM/YYYY"
                    onChange={(val: string) =>
                      updateEditableField('dateOfBirth', val)
                    }
                    themedColors={themedColors}
                  />
                  <EditRow
                    icon="wc"
                    label={t('idScanner.gender')}
                    value={data.gender}
                    placeholder="Nam/Nữ"
                    onChange={(val: string) =>
                      updateEditableField('gender', val)
                    }
                    themedColors={themedColors}
                  />
                  <EditRow
                    icon="home"
                    label={t('idScanner.address')}
                    value={data.address}
                    placeholder="Số nhà, đường, phường..."
                    onChange={(val: string) =>
                      updateEditableField('address', val)
                    }
                    themedColors={themedColors}
                    multiline
                  />
                  <EditRow
                    icon="location-city"
                    label={t('idScanner.placeOfOrigin') || 'Quê quán'}
                    value={data.placeOfOrigin}
                    placeholder="Tỉnh/Thành phố..."
                    onChange={(val: string) =>
                      updateEditableField('placeOfOrigin', val)
                    }
                    themedColors={themedColors}
                  />
                </>
                ...
                <>
                  ...
                  <InfoRow
                    ...
                    value={data.placeOfOrigin || '—'}
                    themedColors={themedColors}
                    missing={!data.placeOfOrigin}
                    t={t}
                  />
                  {data.oldIdNumber ? (
                    <InfoRow
                      icon="history"
                      label={t('idScanner.oldId')}
                      value={data.oldIdNumber}
                      themedColors={themedColors}
                      t={t}
                    />
                  ) : null}
            ...
            <Text
              style={[styles.noteText, {color: themedColors.textSecondary}]}>
          ...
          <View
            ...
            {editMode ? (
              ...
              <>
                <TouchableOpacity
                  style={[styles.rescanBtn, {borderColor: themedColors.border}]}
                  onPress={handleCancelEdit}>
                  <Icon name="close" size={20} color={themedColors.text} />
                  <Text
                    style={[styles.rescanBtnText, {color: themedColors.text}]}>
                    {t('idScanner.btnCancel')}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
              ...
              <>
                <TouchableOpacity
                  style={[styles.editBtn, {borderColor: themedColors.border}]}
                  onPress={handleEdit}>
                  <Icon name="edit" size={20} color={themedColors.text} />
                  <Text
                    style={[styles.editBtnText, {color: themedColors.text}]}>
                    {t('idScanner.btnEdit')}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
      ...
      <SafeAreaView
        ...
        <View style={styles.cameraContainer}>
            ...
            <View style={styles.noPermissionView}>
              ...
              <Text
                ...
                {cameraError
                  ...
                  : t('idScanner.cameraNoPermission')}
                {'\n'}
          ...
          <View style={styles.overlay}>
            <View
              style={[styles.scanFrame, {borderColor: themedColors.primary}]}>
              {/* 4 góc khung */}
              <View
                style={[
                  styles.corner,
            ...
            <Text style={styles.hintText}>
              {t('idScanner.hint') || 'Đưa mã QR trên CCCD vào khung'}
            </Text>
            ...
            <View
              ...
              <View
                style={[
                  styles.statusDot,
                  {backgroundColor: isCameraActive ? '#4CAF50' : '#FF9800'},
                ]}
              />
              <Text
                style={[
                  styles.statusText,
                  {color: isCameraActive ? '#4CAF50' : '#FF9800'},
                ]}>
                {isCameraActive
                  ? isScanning
                    ? 'Đang chờ quét...'
...
const InfoRow = ({
  ...
  <View style={styles.infoRow}>
    <View style={styles.infoRowLeft}>
      ...
      />
      <Text style={[styles.infoLabel, {color: themedColors.textSecondary}]}>
        {label}
      </Text>
    </View>
    <Text
...
const EditRow = ({
  ...
  <View style={styles.editRow}>
    <View style={styles.editRowLeft}>
      ...
      <Text style={[styles.editLabel, {color: themedColors.textSecondary}]}>
        {label}
      </Text>
    </View>
    <TextInput
