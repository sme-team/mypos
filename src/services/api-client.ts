...
export const AUTH_API_BASE = ENV_AUTH_API_BASE ?? 'http://192.168.1.77:3001';
...
function createAuthApiClient(apiPrefix: string, tag: string): AxiosInstance {
  ...
  instance.interceptors.request.use(
    (config: InternalAxiosRequestConfig) => {
      ...
      const publicEndpoints = ['/login', '/register', '/pre-register', '/verify-email', '/url', '/callback', '/token', '/shop/setup'];
      const isPublicEndpoint = publicEndpoints.some(endpoint =>
        config.url?.includes(endpoint),
      );

      if (isPublicEndpoint) {
        logger.debug(`[${tag}] ➡️ PUBLIC ${config.method?.toUpperCase()} ${config.url}`);
        return config;
      }
