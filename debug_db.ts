...
async function checkUnits() {
    ...
    try {
        const units = await QueryBuilder.table('units', db.getInternalDAO()).select(['id', 'unit_code', 'name']).get();
        console.log("Units:", JSON.stringify(units, null, 2));
        
        const prices = await QueryBuilder.table('prices', db.getInternalDAO()).select(['variant_id', 'unit_id', 'price', 'price_list_name']).limit(10).get();
